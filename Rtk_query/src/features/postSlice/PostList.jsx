  ...
  return (
    <div className=" min-h-screen flex flex-col items-center py-8">
      ...
      {isLoading ? (
        ...
        <div className="p-6 rounded-lg bg-[#1b2025] shadow-lg flex flex-col gap-6 items-center max-w-2xl w-full">
          ...
          <div className="grid grid-cols-1 border-t-2 border-purple-600 border-dashed p-3 md:grid-cols-2 gap-6 font-bold font-mono">
            <p className="text-xl">
              <span className="font-bold bg-clip-text text-transparent bg-gradient-to-br from-green-600 to-white p-2 font-mono">Easy</span>: <span>{data ? `${data.easySolved} out of ${data.totalEasy}` : "400 out of 600"}</span> 
            </p>
            <p className="text-xl">
              <span className="font-bold bg-clip-text text-transparent bg-gradient-to-br from-amber-500 to-white p-2 font-mono">Medium</span>: <span>{data ? `${data.mediumSolved} out of ${data.totalMedium}` : "500 out of 1200"}</span>
            </p>
            <p className="text-xl">
              <span className="font-bold bg-clip-text text-transparent bg-gradient-to-r from-red-800 to-white p-2 font-mono">Hard</span>: <span> {data ? `${data.hardSolved} out of ${data.totalHard}` : "100 out of 300"}</span>
            </p>
            <p className="text-xl">
              <span className="font-bold text-[#7471f7] p-1 font-mono">Acceptance Rate</span>: <span>{data ? data.acceptanceRate : "85%"}</span> 
            </p>
            <p className="text-xl">
              <span className="font-bold text-[#7471f7] p-1 font-mono">Ranking</span>: <span>{data ? data.ranking : "12,345"}</span> 
            </p>
            <p className="text-xl">
              <span className="font-bold text-[#7471f7] p-1 font-mono">Contribution Points</span>: <span>{data ? data.contributionPoints : "250"}</span> 
            </p>
            <p className="text-xl">
              <span className="font-bold text-[#7471f7] p-1 font-mono">Likes</span>: <span>{data ? data.reputation : "50"}</span> 
            </p>
          </div>
        </div>
      ) : (
